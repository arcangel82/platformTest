...
/**
 * Service implementing the price retrieval use case.
 * Contains the business logic for selecting the appropriate price.
 */
@Service
public class PriceService implements GetPriceUseCase {
  ...
  public Price getApplicablePrice(Long brandId, Long productId, LocalDateTime applicationDate) {
    logger.debug("Getting applicable price for brand: {}, product: {}, date: {}",
    ...
    if (applicablePrices.isEmpty()) {
      logger.warn("No price found for brand: {}, product: {}, date: {}",
              brandId, productId, applicationDate);
    ...
    logger.debug("Selected price with priority {} and value {}",
            selectedPrice.getPriority(), selectedPrice.getPrice());

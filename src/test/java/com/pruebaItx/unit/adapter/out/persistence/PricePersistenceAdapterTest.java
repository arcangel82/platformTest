...
class PricePersistenceAdapterTest {
    ...
    void shouldFindApplicablePricesSuccessfully() {
        ...
        // When
        List<Price> result = pricePersistenceAdapter.findApplicablePrices(brandId, productId, applicationDate);
    ...
    void shouldHandleDifferentBrandIds() {
        ...
        // When
        List<Price> result = pricePersistenceAdapter.findApplicablePrices(differentBrandId, productId, applicationDate);
    ...
    void shouldHandleDifferentProductIds() {
        ...
        // When
        List<Price> result = pricePersistenceAdapter.findApplicablePrices(brandId, differentProductId, applicationDate);
    ...
    void shouldHandleDifferentApplicationDates() {
        ...
        // When
        List<Price> result = pricePersistenceAdapter.findApplicablePrices(brandId, productId, differentDate);
    ...
    void shouldPreserveOrderFromRepository() {
        ...
        List<PriceEntity> entitiesInOrder = Arrays.asList(priceEntity2, priceEntity1); // Higher priority first
        when(priceJpaRepository.findApplicablePrices(brandId, productId, applicationDate))
    ...
    private List<PriceEntity> createManyPriceEntities(int count) {
        return java.util.stream.IntStream.range(0, count)
                .mapToObj(i -> createPriceEntity((long) i, i, new BigDecimal("10.00").add(new BigDecimal(i))))
